...
import { useLogoutMutation } from '../../../features/auth/signin/store/signInApi';
  ...
  return (
    <Box p={6} borderColor="gray.100" h={{ base: 'auto', md: '100%' }} minH={{ base: 'auto', md: '400px' }} display="flex" flexDirection="column">
      ...
      <Box borderTop="1px solid" borderColor="gray.100" mt={6} pt={6}>
        <Button  variant="ghost"  leftIcon={<LogOut size={18} />}  w="full"  justifyContent="flex-start"  color="gray.600"  fontWeight="400" 
         _hover={{ bg: 'red.50', color: 'red.600' }}  _active={{ bg: 'red.100' }}  onClick={handleLogout}  isLoading={isLoggingOut}  isDisabled={isLoggingOut}>
          Logout
        </Button>
      </Box>
    </Box>
