...
import { useGetStorefrontQuery, useUpdateStorefrontMutation,} from './store/storeFrontApi';
import HueSlider from './HueSlider';
import useStorefrontForm from '../../../hooks/useStorefrontForm';
import useStorefrontLogoUpload from '../../../hooks/useStorefrontLogoUpload';
...
export default function StorefrontSettings() {
	...
	const { brandColor, isAnnouncementActive, announcementText, logoPreview, savedState, maxLength, charCount, isOverLimit, 
  setBrandColor, setIsAnnouncementActive, setAnnouncementText, setLogoFile, setLogoPreview, hasChanges, discard, } = useStorefrontForm(storefrontRes);
	const { fileInputRef, triggerFileUpload, handleLogoUpload, isUploadingLogo } =
		useStorefrontLogoUpload({
			setLogoFile,
			setLogoPreview,
			savedLogoPreview: savedState.logoPreview,
		});
	...
	const handleSave = async () => {
		if (charCount > maxLength) {
			notify('error', 'Character limit exceeded', `Announcement text must be ${maxLength} characters or less.`);
			return;
		}
		...
		try {
			...
			notify('success', 'Changes saved', res?.message || 'Your storefront settings have been updated.');
		} catch (err) {
			notify('error', 'Error', err?.data?.message || 'Failed to save storefront settings.');
		}
	};
	...
	return (
		<Box w="full">
			<VStack align="stretch" spacing={1} mb={8}>
				<PageHeader title="Storefront Settings" description="Manage your store's appearance, domain, and search engine presence."	mb="0"/>
			</VStack>
				...
				<VStack align="stretch" spacing={6}>
					...
					<Stack direction={{ base: 'column', md: 'row' }} spacing={{ base: 6, md: 8 }}>
						<Box flex={1}>
							...
							<Box	border="2px dashed"	borderColor="#D4A373"	borderRadius="lg"	bg="white"	
              p={8}	textAlign="center"	cursor="pointer"	onClick={triggerFileUpload}	transition="all 0.2s"	_hover={{ bg: 'gray.50', borderColor: '#A94F00' }}>
			...
			<Box bg="white" borderRadius="xl" p={6} mb={8} boxShadow="sm" border="1px solid" borderColor="gray.100">
				<VStack align="stretch" spacing={6}>
					<Flex justify="space-between" align="center">
						...
						<Switch
							size="lg"
							isChecked={isAnnouncementActive}
			...
			<Flex justify="flex-end" gap={3} mt={6}>
				...
				<Button
					...
					onClick={handleSave}
					isLoading={isSaving}
					isDisabled={!hasChanges() || isOverLimit}
				>
					Save Changes
				</Button>
			</Flex>
